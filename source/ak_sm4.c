/* ----------------------------------------------------------------------------------------------- */
/*  Использованы исходные коды: Axel Kenzo,          axelkenzo@mail.ru                             */
/*                              Mikhail Lavrinovich, mikhail.lavrinovich@netcracker.com            */
/*  а также реализация SM4 из OpenSSL                                                              */
/*                                                                                                 */
/*  Файл ak_sm4.h                                                                                  */
/*  - содержит реализацию алгоритма блочного шифрования SM4                                        */
/* ----------------------------------------------------------------------------------------------- */
#include <ak_bckey.h>
#include <ak_tools.h>
...
static const ak_uint8 SM4_S[256] = {
...
static ak_uint32 rotl(ak_uint32 a, ak_uint8 n) {
  return (a << n) | (a >> (32 - n));
}

static ak_uint32 load_u32_be(const ak_uint8 *b, ak_uint32 n) {
  return ((ak_uint32)b[4 * n] << 24) | ((ak_uint32)b[4 * n + 1] << 16) |
         ((ak_uint32)b[4 * n + 2] << 8) | ((ak_uint32)b[4 * n + 3]);
}

static void store_u32_be(ak_uint32 v, ak_uint8 *b) {
  b[0] = (ak_uint8)(v >> 24);
  b[1] = (ak_uint8)(v >> 16);
  b[2] = (ak_uint8)(v >> 8);
  b[3] = (ak_uint8)(v);
...
static ak_uint32 SM4_T_slow(ak_uint32 X) {
  ak_uint32 t = 0;

  t |= ((ak_uint32)SM4_S[(ak_uint8)(X >> 24)]) << 24;
  t |= ((ak_uint32)SM4_S[(ak_uint8)(X >> 16)]) << 16;
  t |= ((ak_uint32)SM4_S[(ak_uint8)(X >> 8)]) << 8;
  t |= SM4_S[(ak_uint8)X];
...
static ak_uint32 SM4_T(ak_uint32 X) {
  return SM4_SBOX_T[(ak_uint8)(X >> 24)] ^
         rotl(SM4_SBOX_T[(ak_uint8)(X >> 16)], 24) ^
         rotl(SM4_SBOX_T[(ak_uint8)(X >> 8)], 16) ^
         rotl(SM4_SBOX_T[(ak_uint8)X], 8);
}
...
int SM4_set_key(const ak_uint8 *key, SM4_KEY *ks) {
   ...
   * Family Key
   */
  static const ak_uint32 FK[4] = {0xa3b1bac6, 0x56aa3350, 0x677d9197,
                                  0xb27022dc};

  /*
   * Constant Key
   */
  static const ak_uint32 CK[32] = {
...
static void ak_sm4_encrypt(ak_skey skey, ak_pointer in, ak_pointer out) {
  ak_uint32 B0 = load_u32_be(in, 0);
  ak_uint32 B1 = load_u32_be(in, 1);
  ak_uint32 B2 = load_u32_be(in, 2);
  ak_uint32 B3 = load_u32_be(in, 3);
  ...
  store_u32_be(B3, (ak_uint8 *)out);
  store_u32_be(B2, (ak_uint8 *)out + 4);
  store_u32_be(B1, (ak_uint8 *)out + 8);
  store_u32_be(B0, (ak_uint8 *)out + 12);
}
